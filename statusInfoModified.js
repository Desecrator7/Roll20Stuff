...
var StatusInfo = StatusInfo || (function() {
    ...
    getCampaignMarkers = () => {
        ...
        for(var i in campaignMarkers) {
            icon_images.push({id:campaignMarkers[i].id, name: campaignMarkers[i].tag.toLowerCase(), url: campaignMarkers[i].url});
            markers.push(campaignMarkers[i].tag.toLowerCase());
        }
    },
    ...
    //reconfigured getIcon() to work with data from getCampaignMarkers()
    getIcon = (icon, style='') => {
        let X = '';
        let iconStyle = ''
        let ident = 0;
        ...
        for(var i in icon_images){
            if(icon_images[i].name == icon)
            {
                ident = icon_images[i].id;
                link = icon_images[i].url;
            }
        }
        ...
        if(ident > 0) {
            ...
            iconStyle += 'background-color: ' + link + ';';
